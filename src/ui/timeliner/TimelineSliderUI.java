 ...
 * uses the TimelineLookAndFeel.
 *
 * visual adjustments can be made in paintTrack and paintThumb
 * other parts that may need adjusting are getThumbSize, getThumbOverhang, and calculateTrackRect
 * event handling may be adjusted in ChangeHandler and TrackListener
 *
 ...
 */
 ...
 public class TimelineSliderUI extends SliderUI {
    ...
    private static final Dimension PREFERRED_HORIZONTAL_SIZE = new Dimension(200, 18); // was 21
    private static final Dimension PREFERRED_VERTICAL_SIZE = new Dimension(21, 200);
    private static final Dimension MINIMUM_HORIZONTAL_SIZE = new Dimension(36, 18); // was 21
    private static final Dimension MINIMUM_VERTICAL_SIZE = new Dimension(21, 36);

    private transient boolean isDragging;
    private transient boolean draggingTimepoint;
    private transient boolean draggingMarker;
    private transient boolean isVisibleThumb;
   ...
   protected Color getShadowColor() {
    ...
    // ComponentUI Interface Implementation methods
     ...
     public static ComponentUI createUI(JComponent c)    {
        return new TimelineSliderUI((JSlider)c);
    }

    public TimelineSliderUI(JSlider c) {
    }

    public void installUI( JComponent c ) {
        slider = (TimelineSlider) c;

        slider.setEnabled(slider.isEnabled());
        slider.setOpaque(false);

        // a few timeline specific variables
        isDragging = false;
        ...
        leftToRightCache = TimelineLookAndFeel.isLeftToRight(slider);
        ...
        thumbColor = slider.getBackground();
    }
    ...
    protected void calculateThumbLocation() {
        ...
        if ( slider.getOrientation() == JSlider.HORIZONTAL ) {
            int valuePosition = xPositionForValue(slider.getValue(), trackRect.width);
    ...
    protected void calculateTrackRect() {
        if ( slider.getOrientation() == JSlider.HORIZONTAL ) {
            ...
            trackRect.height = thumbRect.height - 10; // adjusted
        }
        else {
            if(TimelineLookAndFeel.isLeftToRight(slider)) {
    ...
    protected void recalculateIfOrientationChanged() {
        boolean ltr = TimelineLookAndFeel.isLeftToRight(slider);
           ...
           g.translate( 0, tickBounds.y);
            ...
            if ( slider.getMinorTickSpacing() > 0 ) {
                while ( value <= slider.getMaximum() ) {
                    xPos = xPositionForValue( value, trackRect.width);
    ...
    public void paintThumb(Graphics g)  {
      if (isVisibleThumb) {
        ...
        int h = knobBounds.height - 1; // minor adjustment
        ...
        if ( !slider.getPaintTicks() ) {
         ...
         /**
         * here is the code to alter to change the appearance of the current thumb
         */

          // draggable box
          g.setColor(thumbColor);
          g.fillRect(0, 11, w, h-11);

          // needle
          g.drawLine(w/2-1, 11, w/2-1, 5);
          g.setColor(Color.black);
          g.drawLine(w/2, 10, w/2, 0);

          // outline
          g.setColor(Color.black);
          g.drawLine(0, h-1, w-1, h-1);
          g.drawLine(w-1, 11, w-1, h-1);

          // highlight
          g.setColor(highlightColor);
          g.drawLine(0, 11, 0, h-2);

          // shadow
          g.setColor(shadowColor);
          g.drawLine(1, h-2, w-2, h-2);
          g.drawLine(w-2, 10, w-2, h-3);
     ...
     * a timepoint on the timeline, which needs to be draggable
     */
    ...
    public class ChangeHandler implements ChangeListener {
        public void stateChanged(ChangeEvent e)                {
            if ( !isDragging ) {
                ...
                // painting code that is specific to the timeline panel
                if (slider.pnlTimeline != null) {
                  slider.pnlTimeline.sliderRepainted = true;
                  Rectangle displayRect = slider.pnlTimeline.getVisibleRect();
                  slider.repaint( trackRect.x - 10, unionRect.y, trackRect.width + 20, unionRect.height);

                 // scroll the display if the thumb goes off the end of the screen
                 int currX = thumbRect.x + slider.pnlTimeline.getTimeline().getLineStart();
                 if ((currX > (displayRect.getX() + displayRect.getWidth())) || (currX < displayRect.getX())) {
                    slider.pnlTimeline.scrollRectToVisible(new Rectangle(unionRect.x, (int)(slider.pnlTimeline.getHeight())-10, (int)displayRect.getWidth(), 10));
                    slider.repaint( trackRect.x - 10, unionRect.y, trackRect.width + 20, unionRect.height);
                 }
                }
            }
    ...
    public class TrackListener extends MouseInputAdapter {
        ...
        */
        public void mousePressed(MouseEvent e)                {
            ...
            /**
             * I have altered the behavior here by substituting the method Jump for Scroll
             * Update: now I am commenting it out so that I can call it from outside the UI
             * this allows me to either jump or not
             */
    ...
    public void setVisibleThumb(boolean state) {
      this.isVisibleThumb = state;
    }
}
